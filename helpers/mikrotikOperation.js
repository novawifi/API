...
class Mikrotik {
    async addSecret(channel, { name, password, service = 'pppoe', profile = 'default' }) {
    ...
    async addHotspotServerProfile(
        ...
        if (macCookie !== undefined && macCookie !== null) args.push(`=mac-cookie=${macCookie}`);
        if (macCookieTimeout) args.push(`=mac-cookie-timeout=${macCookieTimeout}`);
