...
class DataBase {
    ...
    async expireHotspotUserByUsername(platformID, username) {
    ...
    async upsertRadiusUser({ username, password, groupname, rateLimit, dataLimitBytes, expireAt, period, sessionTimeoutSeconds }) {
        ...
        try {
            const parsePeriodToSeconds = (periodValue) => {
                ...
                if (!text || text === "noexpiry" || text === "no expiry" || text === "unlimited") return null;
                const match = text.match(/^(\d+)\s*(minute|min|hour|hr|day|month|year)s?$/i);
            ...
            const formatExpiration = (dateValue) => {
            ...
            if (Number(sessionTimeoutSeconds) > 0) {
                timeoutSeconds = Math.floor(Number(sessionTimeoutSeconds));
            } else if (expireAt) {
                const expiryDate = new Date(expireAt);
                if (!Number.isNaN(expiryDate.getTime())) {
                    timeoutSeconds = Math.floor((expiryDate.getTime() - Date.now()) / 1000);
                }
            } else {
                timeoutSeconds = parsePeriodToSeconds(period);
            }
    ...
    async updateMpesaCodeIfNotComplete(code, data) {
        if (!code || !data) return { updated: 0 };
        try {
            const nextData = { ...data };
            if (typeof nextData.charges === "number") {
                nextData.charges = nextData.charges.toFixed(2);
    ...
    async getB2BPaymentsByPlatformPaged(platformID, limit = 100, offset = 0) {
        if (!platformID) return null;
        try {
            const payments = await prisma.mpesa.findMany({
                where: {
            ...
            console.error("Error fetching paged B2B payments by platform:", error);
            throw error;
        }
    }
    ...
    async getDuplicateActiveUserCodes(platformID) {
        ...
        try {
            ...
            return rows
                .filter((row) => (row._count?.code || 0) > 1)
                .map((row) => row.code)
                .filter((code) => code && code !== "null");
        } catch (error) {
            console.error("Error getting duplicate active codes:", error);
            return [];
        }
    }

    async getActiveUsersByCodes(platformID, codes = []) {
        if (!platformID || !Array.isArray(codes) || codes.length === 0) return [];
        try {
            const users = await prisma.user.findMany({
                where: {
                    platformID,
            ...
            console.error("Error getting active users by codes:", error);
            return [];
        }
    }
    ...
    async getStationByWebfigHost(mikrotikWebfigHost) {
        if (!mikrotikWebfigHost) return null;
        try {
    ...
    async getUsersByPlatformWithPackage(platformID) {
        if (!platformID) return null;
        try {
            const users = await prisma.user.findMany({
                where: { platformID },
    ...
    async getDashboardStatsBundle(platformID) {
        ...
        try {
            ...
            const stationPackageIds = new Set(packages.map((pkg) => pkg.id));
            const stationPackageIdList = Array.from(stationPackageIds);
            const revenueWhere = {
                platformID,
                status: "COMPLETE",
                service: {
                ...
                OR: [
                    { reason: { in: stationPackageIdList } },
                    { reason: null },
                    { reason: "" },
                ],
            };
    ...
    async reconcileDashboardRevenueStats(platformID) {
        if (!platformID) return null;
        const now = new Date();
        const startOfToday = new Date(now);
        startOfToday.setHours(0, 0, 0, 0);
        ...
        const baseWhere = Prisma.sql`"platformID" = ${platformID}
            AND "status" = 'COMPLETE'
            AND "service" NOT IN ('bill','sms','Mpesa B2B')`;

        const [platformRow] = await prisma.$queryRaw(Prisma.sql`
            SELECT
                COALESCE(SUM(CAST("amount" AS DOUBLE PRECISION)), 0) AS "total",
                COALESCE(SUM(CAST("amount" AS DOUBLE PRECISION)) FILTER (WHERE "createdAt" BETWEEN ${startOfToday} AND ${endOfToday}), 0) AS "daily",
                COALESCE(SUM(CAST("amount" AS DOUBLE PRECISION)) FILTER (WHERE "createdAt" BETWEEN ${startOfYesterday} AND ${endOfYesterday}), 0) AS "yesterday",
        ...
        const existing = await this.getDashboardStats(platformID);
        if (!existing) {
            const payload = await this.buildDashboardStatsPayload(platformID);
            ...
            stats.totalRevenue = Number(platformRow?.total || 0);
            stats.dailyRevenue = Number(platformRow?.daily || 0);
            stats.yesterdayRevenue = Number(platformRow?.yesterday || 0);
            stats.thismonthRevenue = Number(platformRow?.thisMonth || 0);
            stats.lastmonthRevenue = Number(platformRow?.lastMonth || 0);
            await this.upsertDashboardStats(platformID, {
                stats,
        ...
        const stations = await prisma.station.findMany({
            where: { platformID },
            select: { id: true, mikrotikHost: true },
        });
        if (!stations || stations.length === 0) return true;

        const packages = await prisma.package.findMany({
            where: { platformID },
            select: { id: true, routerHost: true },
        });
        const packagesByHost = new Map();
        for (const pkg of packages) {
            if (!pkg.routerHost) continue;
            const list = packagesByHost.get(pkg.routerHost) || [];
            list.push(pkg.id);
        ...
        for (const station of stations) {
            ...
            if (ids.length === 0) {
                ...
                if (existingStation) {
                    ...
                    await this.upsertStationDashboardStats(platformID, station.id, {
                        stats,
                        funds: existingStation.funds || {},
                        networkUsage: existingStation.networkUsage || [],
                        isB2B: existingStation.isB2B || false,
                    });
                } else {
                    await this.rebuildStationDashboardStats(platformID, station.id);
                }
                continue;
            }
            ...
            const stats = { ...(existingStation.stats || {}) };
            stats.totalRevenue = Number(stationRow?.total || 0);
            stats.dailyRevenue = Number(stationRow?.daily || 0);
            stats.yesterdayRevenue = Number(stationRow?.yesterday || 0);
            stats.thismonthRevenue = Number(stationRow?.thisMonth || 0);
            stats.lastmonthRevenue = Number(stationRow?.lastMonth || 0);
            await this.upsertStationDashboardStats(platformID, station.id, {
                stats,
                funds: existingStation.funds || {},
                networkUsage: existingStation.networkUsage || [],
    ...
    async getStationDashboardStatsBundle(platformID, stationId) {
        ...
        try {
            ...
            const stationPackages = await prisma.package.findMany({
                where: { platformID, routerHost: station.mikrotikHost || "" },
                select: { id: true },
            });
            ...
            const [
                ...
                prisma.user.findMany({
                    where: {
                        platformID,
                        status: "active",
                        package: { routerHost: station.mikrotikHost || "" },
                    },
                }),
                prisma.pppoe.findMany({
    ...
    async searchPackages({ platformID, search, limit, offset }) {
        ...
        const categoryMatch = categoryLookup[normalized] || null;
        const where = {
            platformID,
            ...(search && {
                OR: [
                    ...
                    ...(categoryMatch ? [{ category: { equals: categoryMatch } }] : []),
    ...
    async getPublishedBlogs(limit = 100) {
        try {
            return await prisma.blog.findMany({
    ...
    async getPlatformLastBillPayment(platformID) {
        if (!platformID) return null;
        try {
            ...
            console.error("Error fetching last bill payment:", error);
            throw error;
        }
    }

    async getPlatformLastBillPaymentForBill(platformID, billId) {
        if (!platformID || !billId) return null;
        try {
            return await prisma.mpesa.findFirst({
                where: {
                    platformID,
            ...
            console.error("Error fetching last bill payment for bill:", error);
            throw error;
        }
    }

    async getPlatformLastBillPaymentByAmounts(platformID, amounts) {
        if (!platformID || !Array.isArray(amounts) || amounts.length === 0) return null;
        const variants = [];
